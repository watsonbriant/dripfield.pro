...
export function JiveCompleteShowsList({ onProgressUpdate }: JiveCompleteShowsListProps) {
    const navigate = useNavigate();
    const { user } = useAuth();
    const [shows, setShows] = useState<ShowWithJive[]>([]);
    ...
    async function fetchShows() {
        try {
            ...
            // Get count of shows with jive complete = true
            const { count: showCount, error: countError } = await supabase
                .from('shows')
                .select('*', { count: 'exact', head: true })
                .eq('show_jivecomplete', true);
            ...
            const formattedShows = allShows.map(show => ({
                ...
                show_gap: show.show_gap !== null && show.show_gap !== undefined
                    ...
                    : null,
                show_jivecomplete: show.show_jivecomplete
            }));
